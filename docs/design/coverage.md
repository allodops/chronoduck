...
| topk/bottomk re-evaluated per grid point | TraceQL topk; MetricsQL topk_min/max/avg/last; ClickHouse timeSeriesTopKMasks/BottomKMasks/LimitKMasks | D | QUALIFY row_number() OVER (PARTITION BY t ORDER BY v) |
...
| rate / increase using the last sample before the window, no extrapolation | MetricsQL rate/increase/delta; Timescale counter_agg delta; PromQL Anchored vector selector (experimental, main) | K | ANCHOR edge mode (edge_context anchor at or before t−w) |
...
| Continuous aggregates / downsampling tasks / TSDS downsampling | Timescale; InfluxDB; Elastic; Thanos | S | GROUP BY time_bucket over _state into a table; DuckLake/RawDuck keep it |
