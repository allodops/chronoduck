...
Port of scripts/lib/gh-diff.mjs. Run by a human/Claude Code on the owner's
machine (never from a CI workflow -- CI has no gh-tsouza identity installed;
scripts/lib/gh.py's plain-`gh` helpers are the CI-safe equivalent). Used by
both scripts/pr-hygiene.py and scripts/hygiene/forbid-deferral.py once
#241-#244 port those callers (#166 DRY: this fetch, including its rationale,
used to be duplicated verbatim across both).
