...
GRID_STEP_US = 1_000_000  # 1s, the shape's own grid resolution
WINDOW_US = 300_000_000  # 5 minutes, the shape's own window
GRID_RANGE_S = 3600  # scaled down from 30 days -- see header comment
TOTAL_ROWS = 20_000_000  # held constant across both configs
LOW_SERIES = 24  # the PR's own "low" series count
HIGH_SERIES = 20_000  # pushed toward the documented ~10k-series target
MAX_GROWTH_RATIO = 1.1  # generous slack above the ~1.01x measured baseline,
# well below the ~1.16x measured for the named leak mutant (see header)


def build_sql(series_count):
    return f"""
LOAD '{EXTENSION_PATH}';
SET threads TO 1;
CREATE TEMP TABLE samples AS
SELECT
    (i % {series_count})::UBIGINT AS series_id,
    TIMESTAMP '2024-01-01 00:00:00' + INTERVAL ((i // {series_count})) SECOND AS ts,
...
def measure_peak_kb(sql_path):
    with open(sql_path, "rb") as stdin_file:
        proc = subprocess.run(
            [TIME_BIN, "-v", DUCKDB_BIN, "-unsigned"],
            stdin=stdin_file,
    ...
    match = re.search(r"Maximum resident set size \(kbytes\):\s*(\d+)", err)
    if not match:
        raise RuntimeError(f'could not find "Maximum resident set size" in /usr/bin/time -v output:\n{err}')
    return int(match.group(1))


def main():
    if not Path(DUCKDB_BIN).exists() or not Path(EXTENSION_PATH).exists():
        print("memory-check-grid-stream: FAIL", file=sys.stderr)
        print(f"  {DUCKDB_BIN} / {EXTENSION_PATH} not found -- run `make release` first", file=sys.stderr)
        sys.exit(1)
    if not Path(TIME_BIN).exists():
        print("memory-check-grid-stream: FAIL", file=sys.stderr)
        print(f"  {TIME_BIN} not found (GNU time with -v support is required)", file=sys.stderr)
        sys.exit(1)

    with tempfile.TemporaryDirectory(prefix="memory-check-grid-stream-") as tmp:
        low_sql_path = Path(tmp) / "low.sql"
        high_sql_path = Path(tmp) / "high.sql"
        low_sql_path.write_text(build_sql(LOW_SERIES))
        high_sql_path.write_text(build_sql(HIGH_SERIES))

        print(
            f"memory-check-grid-stream: running {LOW_SERIES}-series config ({TOTAL_ROWS} total rows, "
            "1h/1s/5min shape)...",
            file=sys.stderr,
        )
        low_kb = measure_peak_kb(low_sql_path)
        print(f"memory-check-grid-stream: {LOW_SERIES} series -> peak RSS {low_kb} KB", file=sys.stderr)

        print(
            f"memory-check-grid-stream: running {HIGH_SERIES}-series config ({TOTAL_ROWS} total rows, "
            "1h/1s/5min shape)...",
            file=sys.stderr,
        )
        high_kb = measure_peak_kb(high_sql_path)
        print(f"memory-check-grid-stream: {HIGH_SERIES} series -> peak RSS {high_kb} KB", file=sys.stderr)

    ratio = high_kb / low_kb
    print(
        f"memory-check-grid-stream: {HIGH_SERIES // LOW_SERIES}x more series -> "
        f"{ratio:.3f}x peak RSS (bound: <= {MAX_GROWTH_RATIO}x)",
        file=sys.stderr,
    )
    ...
    if ratio > MAX_GROWTH_RATIO:
        print("memory-check-grid-stream: FAIL", file=sys.stderr)
        print(
            f"  peak RSS grew {ratio:.3f}x for a {HIGH_SERIES // LOW_SERIES}x increase in series count -- "
            "resident state is tracking series count, not just threads * window (docs/testing/memory.md, "
            "issue #40 acceptance criterion 2)",
            file=sys.stderr,
        )
        sys.exit(1)
    print("memory-check-grid-stream: PASS")
